...
- Often need to forecast more than a single object
    - Need to know multiple aspects of future situation to make policy or decisions
        - Ideal monetary policy depends on future state of both inflation and unemployment
...
- Simplest way to build a class of multivariate forecast rules is to combine univariate classes
    - $\mathcal{F}=\times_{i=1}^{m}\mathcal{F}_i$, where $\mathcal{F}_i=\{f:\mathbf{Z}\to\mathbb{R}\}$ is a univariate class like the linear classes from before, is called **separable**
...
##Obtain and transform NIPA Data (cf Lecture 06)
...
## Exercise: Money and Income

- Check out [Kaggle notebook](https://www.kaggle.com/davidchilders/multivariate-forecasts) to try a VAR forecast for yourself
