...
ggdag(yxdag)+theme_dag_blank()+ggplot2::labs(title="X causes Y",subtitle="X is a parent of Y, Y is a child of X") #Plot causal graph
```
...
ggdag(yxzdag)+theme_dag_blank()+ggplot2::labs(title="Observed Common Cause Z") #Plot causal graph
```
```{r, fig.height=2.5}
...
ggdag(yxydag)+theme_dag_blank()+ggplot2::labs(title="Unobserved Common Cause") #Plot causal graph
```
